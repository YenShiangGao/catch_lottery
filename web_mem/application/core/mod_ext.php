...
class mod_ext extends mod_base {
	...
	/*修改備份*/
	private function BackUpModify($tb,$ary){
		$raw = $this->get_by($tb,$ary);
		if(count($raw) > 0){
			$raw = json_encode($raw);
			$Conditions = json_encode($ary);
			$this->add_by("backup_modify",array(
	...
	/*刪除備份*/
	private function BackUpDelete($tb,$ary){
		$raw = $this->get_by($tb,$ary);
		if(count($raw) > 0){
			$raw = json_encode($raw);
			$Conditions = json_encode($ary);
			$this->add_by("backup_delete",array(
				"Conditions" 	=> $Conditions , 
				"TableName" 	=> $tb , 
				"Content" 		=> $raw , 
				"SetupTime" 	=> date("Y-m-d H:i:s")
